...
public class HopefullyLessSketchGoalSideAuto extends OpMode {
    ...
    private final double POST_VOLLEY_DELAY = 0.25; // Time for FORWARD purge (forcing the final ball out)
    ...
    public void autonomousPathUpdate() {
        ...
        switch (pathState) {
            ...
            case 2:
                ...
                if (!follower.isBusy()) {
                    shooter.stopShooter();
                    intake.intakeIdle(); // <<< CHANGED from intakeRetainBalls() to intakeIdle()
            ...
            case 6:
                ...
                if (!follower.isBusy()) {
                    shooter.stopShooter();
                    intake.intakeIdle(); // Idle during travel
                    intake.setServoPower(0);
    ...
    public void shootVolley() {
        ...
        // NEW PURGE CONSTANT: Time to spin the intake OUT (reverse) to clear internal mechanism.
        final double POST_VOLLEY_DELAY_REVERSE = 0.15;
        // The total time required for both forward and reverse purges.
        final double TOTAL_PURGE_TIME = POST_VOLLEY_DELAY + POST_VOLLEY_DELAY_REVERSE;


        // 1. VOLLEY COMPLETE PHASE (Includes Purge/Clear)
        if (ballsToShoot == 0) {

            // Phase 1: Forward Purge (POST_VOLLEY_DELAY duration)
            if (elapsedTime < POST_VOLLEY_DELAY) {
                // Keep the shooter running, force intake IN and servo ON to **FORCE THE FINAL BALL OUT**.
                intake.intakeIn();
                intake.setServoPower(1);
                telemetry.addData("Purge State", "1. Forward Clear (0s to " + POST_VOLLEY_DELAY + "s)");

            }
            // Phase 2: Reverse Purge (POST_VOLLEY_DELAY_REVERSE duration)
            else if (elapsedTime < TOTAL_PURGE_TIME) {
                // Intake OUT to clear any fragments or snagged balls from the mechanism.
                intake.intakeOut(); // Spin intake OUT (reverse)
                intake.setServoPower(0); // Servo off or neutral
                telemetry.addData("Purge State", "2. Reverse Clean (" + POST_VOLLEY_DELAY + "s to " + String.format("%.2f", TOTAL_PURGE_TIME) + "s)");
            }

            // Phase 3: Stop and Transition
            else {
                // PURGE IS COMPLETE. Stop everything and transition to the next path state.
                shooter.stopShooter();
                intake.intakeIdle();
                intake.setServoPower(0);
                setPathState();
                telemetry.addData("Shooting Volley", "Purge Complete. Moving to State " + pathState);
            }
        }
        ...
        // 3. FEEDING PHASE
            ...
            if (elapsedTime >= currentShotTime) {
                ...
                if (elapsedTime >= currentShotTime + TIME_PER_SHOT) {
                    ...
                    ballsToShoot--;
                    if (ballsToShoot == 0) {
                        pathTimer.resetTimer(); // Start the purge timer
                    }
                }
            } else {
                // Between shots: servo in the non-feeding state.
                intake.setServoPower(0);
                // Intake motor is idle to prevent excessive internal ball movement.
                intake.intakeIdle();
            }
        }
